...
**Positioning:** Evidence-based groundwater recharge decision guide, grounded in the Sabarkantha case  
**Status:** HUMANIZED / FIELD-LOGIC REFINEMENT COMPLETE — Gate 5 pending user approval  
...
The article should carry one practitioner quote as a recurring anchor:

> **“GIS helps validate the evidence. The decision still has to be made in the field, with the community.”**

A shorter supporting line may still be used where useful:
...
## 2A. Personal-branding system — Water Security Intelligence + author identity

The earlier `weir-web-article` remains the reference for restraint: the technical work stays primary and authorship is visible without turning the article into a portfolio page.

For GWRPZ, the persistent header identity changes from the temporary `KG` mark to the approved **Water Security Intelligence** logo from the user's `dsc-humanizer` repository:

`skills/water-security-intelligence/assets/water-security-intelligence-logo.png`

The logo must be used unchanged.

### Brand hierarchy

**Primary:** `From Mapping to Decision` and the evidence story.  
**Secondary:** Water Security Intelligence logo as the recurring research / water-sector mark.  
**Tertiary:** Kaushal Gadariya author identity and collaboration route.

### Header

Use the approved Water Security Intelligence logo followed by:
...
Hero authorship remains visually secondary:

> **Kaushal Gadariya** · Soil and Water Conservation Engineer · Groundwater · GIS · Watershed & NRM

### SEO / share identity

Recommended production metadata:

- `<title>`: `From Mapping to Decision | Kaushal Gadariya`
- author: `Kaushal Gadariya`
- OpenGraph title: `From Mapping to Decision`
- OpenGraph site name: `Water Security Intelligence — Kaushal Gadariya`
- canonical: final GitHub Pages URL.

### Visual identity

Retain the research-family palette:
- deep navy;
- warm paper;
- teal;
- aqua;
- restrained caution accent.

GWRPZ remains map-led and field-oriented; the Water Security Intelligence logo provides continuity across the author's water-sector analytical work.

### Author card

Use a direct author card:
- `Kaushal Gadariya`
- `Soil and Water Conservation Engineer`
- short provenance paragraph;
- LinkedIn;
- Portfolio;
- one collaboration button: `Discuss a technical collaboration`.

Remove the extra `Explore applied work` CTA button. Portfolio remains available as a normal author link.
...
- keep references available without giving them visual priority over the story.

**Public UX rule:** references stay collapsed by default behind a compact `References & further reading` disclosure. Expanding it reveals the selected papers below. The full research register remains available separately for readers who want more depth.
...
**Display rule in the public article:** do not devote a full open section to these references. Show a single compact `References & further reading` row. On click/tap, expand the selected references below; otherwise keep them hidden. The detailed register remains a technical appendix / downloadable reference.
...
### Recommended author links
- LinkedIn: `https://www.linkedin.com/in/kaushal-gadariya-670221b1/`
- Portfolio: `https://knightfox789.github.io/kaushal-gadariya-portfolio/`

Use these as the primary professional routes. Do not add an aggressive lead form in the first release.
...
## 13A. Humanization and field-language rule

The v0.5 revision applies the DSC Humanizer / Kaushal Voice more strictly.

### Avoid repetitive contrast writing

Do not repeatedly build sentences around `not`, `does not`, `cannot` or a defensive disclaimer immediately after every finding.

Write the main story affirmatively:
- what the evidence shows;
- how it is used;
- what field question follows;
- where the method note sits.

Material caveats remain available in compact **Method notes** and the evidence record.

### Main-story language

Prefer:
- `The analysis uses the nearest mapped drainage segment and retains distance.`
- `The setting is used to compare siting context.`
- `GWRPZ narrows the search area.`
- `Lineaments guide where structural conditions deserve closer field investigation.`

### Lineament interpretation

Mapped lineaments are treated as indicators of possible structural discontinuities such as fractures or fault zones. Where open and hydraulically connected, such features can act as preferential pathways for groundwater movement and recharge.

In planning, the structural setting around the candidate site is more meaningful than exact point-on-line coincidence.

Public display:
- nearest mapped lineament distance;
- field / hydrogeological interpretation.

Method note:
- none of the 58 point geometries coincides exactly with a supplied lineament polyline;
- no undocumented proximity threshold is imposed.

### Main-story caveats moved out of yellow warning cards

The following are no longer displayed as prominent warning boxes:
- dataset-is-not-a-census statement;
- historical class-break reconstruction caveat;
- exact drainage-line intersection caveat;
- `not a performance score` statement.

Where material, these points are integrated naturally or moved to collapsed Method notes.


# 14. Gate 5 decision

This revision supersedes `Web_Article_Content_Architecture_v0.4.md`.
...
Phase 6 should use this v0.4 architecture after approval.