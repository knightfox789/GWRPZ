...
**Positioning:** Evidence-based groundwater recharge decision guide  
...
## 2A. Water Security Intelligence branding interaction

### Header

Use the approved Water Security Intelligence logo asset unchanged:
...
The logo is a compact visual signature. The article title remains the primary text identity.

### Hero

Hero order:
1. eyebrow — `Evidence-based decision guide · Sabarkantha, Gujarat`;
2. H1 — `From Mapping to Decision`;
3. practitioner quote;
4. field-grounded explanatory dek;
5. compact author metadata;
6. `Follow the evidence` CTA.

Remove the former `What this is / What this is not` contrast block. Replace it with one affirmative statement describing the guide.
...
- LinkedIn;
- Portfolio;
- one CTA: `Discuss a technical collaboration`.

Remove `Explore applied work` as a button.
...
4. **Next field / method check**, only where it adds decision value
...
`References & further reading` remains an in-page collapsed disclosure rather than a primary navigation destination. The collaboration action lives in the closing author card rather than the primary nav.
...
## 16A. v0.5 layout and copy refinements

### Evidence summary cards

The four top summary cards use an equal-width grid and aligned vertical centres:
- 58 structures analysed;
- 49 villages represented;
- 5 spatial evidence layers;
- Field + community + engineering.

Metric cards inside chapters use equal minimum height so values and labels align consistently.

### Caveat display

Repeated yellow warning cards are removed from the main story.

Use:
- natural explanatory sentences;
- compact collapsed `Method note` disclosures;
- deeper evidence / QA notes only when the reader asks for them.
...
Main explanation:
mapped lineaments can indicate fractures / fault zones that may provide preferential groundwater pathways when open and hydraulically connected. Their planning role is to guide structural investigation around a candidate site.

The `0/58 exact intersections` diagnostic moves to the collapsed Method note because exact point-on-line coincidence is not the hydrogeological planning objective.


## 17. Gate 5

This v0.5 supersedes `Interaction_Specification_v0.4.md`.
